...
在「设置 → 项目设置 · 视频抽帧」保存当前项目的默认间隔（初始为每 30 个源帧抽一帧）。
设置保存在项目目录的 `my-label-tool.settings.json`，可在首次标注/导出前保存；
不同项目互不影响，单次准备视频仍可临时调整间隔，修改默认值不会重新抽取已有帧。
这是宿主设置文件（[Schema](project-settings.schema.json)），与依赖标注路径的 ProjectConfig 分工独立，
旧项目缺少此文件时使用默认值，不改变 ProjectConfig.schemaVersion 或插件协议。
...
点击「预览插值」后，画布直接以黄色虚线显示尚未应用的图形；当前帧没有插值结果时，
自动跳到第一张待插值帧。可通过面板中的帧按钮或时间轴逐帧检查，预览会随画布缩放和平移。
点击「取消预览」放弃，或「应用插值」写入正式标注；预览本身不修改标注或撤销历史。
应用后的撤销/重做复用既有逐帧历史。操作错误会在 5 秒后自动消失，也可点击关闭；
重新预览成功时立即清除之前的错误提示。
