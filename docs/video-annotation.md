...
撤销/重做右侧的「项目设置」打开独立配置界面；只有已保存或导入项目文件后才能点击。
应用「设置」继续管理快捷键和显示选项。项目默认抽帧方式为 **5 FPS**，也可切换为固定帧间隔。
「保存项目设置」写入当前 `my-label-tool.project.json` 的可选 `settings` 字段；标签、导出路径、
预打标映射和插件配置保留。后续保存标注同样保留设置。
旧项目没有配置时使用 5 FPS；旧 `my-label-tool.settings.json` 仅兼容读取，原帧间隔作为间隔模式保留，
下一次保存项目设置时写入项目文件；已存在的项目文件设置优先。设置结构见 [Schema](project-settings.schema.json)。
不同项目互不影响，单次准备视频仍可临时调整方式/参数，修改默认值不会重新抽取已有帧。
项目设置内的「批量抽取未准备视频」使用已保存的设置逐个处理，已抽帧视频自动跳过。
进度窗口显示完成数与失败详情；取消停止当前及后续抽帧，已完成结果和原有标注保留。
失败项仍是未准备状态，可以修正后再次批量执行。
对已抽帧视频条目右键选择「重新抽帧」，按项目默认方式和参数重建。复用删除确认弹窗，
显示旧帧和标注数量；等待 3 秒后只能鼠标确认，Enter/空格无效，Esc/取消可退出。
新帧准备成功后才把旧帧与元数据移入回收站，并清除对应标注及撤销/重做记录。
失败回滚；其它图片、视频、文件和标注保持原状。抽帧期间可取消，进入回收提交阶段后完成替换。
新帧使用独立目录，旧标注文件重新加载不会误匹配到新帧；再次保存项目可更新磁盘上的标注快照。
左侧「项目素材」按图片/视频显示：点击图片直接标注；点击尚未准备的视频，
在弹窗中选择按 FPS 或固定帧间隔抽帧，确认后准备帧。
也可从菜单「添加视频到项目」选择外部视频；没有打开项目时先选择项目目录。
添加视频保留当前图片、其它视频、标注、标签和撤销历史。
程序在项目目录下新建独立 `video-*` 帧目录，原视频与已有文件均不覆盖。
失败或取消会清理本次未完成目录，当前项目保持原状。
每次解码最长 30 分钟，最多 100 万源帧、1 万抽取帧；超过时分段导入、降低 FPS 或增大间隔。
FPS 支持大于 0 且不超过 1000 的数值（含小数），固定帧间隔为 1–1000000 的整数。
FPS 模式按以首帧为零的时间桶采样，每个非空时间桶取首个源帧；时间桶宽度为 1/FPS。
可变帧率使用实际时间戳，源帧不足或时间断档不重复/补造图片。
...
以解码 PNG 为准。FPS 模式增加可选 targetFps，frameInterval 保留为 1；缺少 targetFps 时
按旧的固定帧间隔规则校验。两种模式均以 frames 中的真实源帧号/时间戳为准，不使用列表序号推算。再次打开项目目录时，自动识别各个直接子目录中的元数据并校验帧序列；
...
格式下拉框统一决定「保存 / 另存为」和 Ctrl+S 的输出，不再提供独立 YOLO 按钮。
首次保存或切换格式后选择该格式的目标文件/目录，后续保存复用该目标；「另存为」始终选择新目标。
内置 JSON/COCO/VOC/YOLO 保存成功后更新项目配置中的格式和目标路径，并保留项目设置。
选择 YOLO TXT 后点击「保存」即可导出图片和全部已抽帧视频的 TXT；不会另写原生 JSON 标注。
每张图片/已抽取视频帧生成一个同名 `.txt`，相对视频帧目录原样保留，空帧写空文件；
根目录生成 `classes.txt`，类别编号从 0 开始，矩形转换为归一化中心点和宽高。
导出仅包含标注文件，不复制图片；未抽帧视频数量在入口下方显示，可先批量抽帧再导出。
非矩形、未知标签、无效尺寸/越界框、同名输出冲突会明确报错，避免静默丢失标注或覆盖文件。
重新打开 YOLO/VOC 项目时按视频子目录分别读取同名帧，沿用项目标签 ID。标准格式只保留各自支持的标注字段，
例如 YOLO 不包含视频轨迹/关键帧属性；需要完整编辑元数据时选择项目 JSON。
