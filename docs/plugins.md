...
Windows 上所有代码型插件都运行在每次进程启动独立创建的 AppContainer 中。容器只获得
插件包、声明使用的系统 Python 解释器运行目录的只读/执行权限，以及宿主创建的标准输入
输出句柄；其他包外文件即使对当前 Windows 用户可读，插件进程也不能直接打开，必须使用
`fs.read` / `fs.write` 宿主代理。宿主环境按白名单重建，不向插件继承 token、密钥或代理
变量。v1 不向 AppContainer 授予 Internet 或私有网络 capability，因此直接 socket 也会
被 Windows 拒绝。环境变量中的离线标志只是兼容常用工具的补充，不是安全边界；
AppContainer 初始化失败时插件拒绝启动，进程结束或启动失败都会撤销临时 ACL 并删除 profile。

### 安全模式与故障隔离

- 安全模式禁用全部代码型插件，但保留纯数据 `label-preset` 插件。
- 单次调用使用 manifest 的 `timeoutMs`（默认 30 秒）；超时会终止插件进程树。
- AppContainer profile 按进程临时创建并自动清理；项目内插件配置仍按既有约定保留。
