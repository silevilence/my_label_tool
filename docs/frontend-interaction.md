...
- 解析器把中键归为 pan（拖拽平移；无位移的单击经 pan 开始/结束路径等效取消草稿），按住空格时左键/中键同样归为 pan——空格门禁复用遮罩栈深度、可编辑焦点与操作互斥三重裁决（`shouldPanWithSpace`，与草稿键盘/快捷键一致）；未支持的按钮归为 null；拖拽阶段仅允许默认模式左键命中图形。滚轮缩放不再要求指针位于图片矩形内，锚点夹取到图片范围。
