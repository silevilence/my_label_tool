...
## 错误提示与画布插值预览复审（2026-09-18）

范围：用户反馈的错误提示一直停留、插值预览仅列出坐标两项问题。
先通过完整 App 的 DOM 集成测试复现：等待 6 秒错误仍存在，成功预览后旧错误未清除且无画布预览。
修复后这些回归通过；不涉及插件契约、数据模型、Tauri commands 或 Rust 逻辑。

| 审核维度 | 结论与证据 |
| --- | --- |
| 需求满足 | 错误 5 秒自动消失、可手动关闭，重新操作清除旧提示；预览自动定位待插值帧，画布显示黄色虚线及未应用标记，支持逐帧检查、取消与应用 |
| 数据与交互质量 | 预览不写 store 或历史；应用后可撤销，修改关键帧使预览失效，切换图片清除预览；矩形、多边形和点均使用原图坐标与画布相同的缩放/平移 |
| 工程规范 | 状态保留在 React，用户文案集中 i18n，无新增依赖；修改文件均未超过 1000 行，未新增插件接口或更改版本 |
| 测试 | 视频组件 16 项通过；包括实际 App 的错误恢复、计时重置、取消、应用/撤销、过期预览、快捷切帧及切换图片回归，三种图形的 SVG 坐标变换测试 |

完整前端 43 个测试文件、277 项通过；既有纯逻辑覆盖率统计口径下行覆盖率 94.51%。
UI 组件与 hooks 不计入该覆盖率百分比，由上述组件/集成测试验证行为。
`npm run typecheck`、`npm run lint`、`cargo clippy --manifest-path src-tauri/Cargo.toml` 均通过。
本次未发现需修复的阻塞项，两项反馈可标记完成；仍按用户要求不使用 Computer Use，
未执行实际桌面标注或视觉验收。
