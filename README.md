...
- **混合项目**：同一项目可包含图片和多个视频，左侧素材列表统一选择；菜单添加视频，打开项目目录自动识别视频及已抽取帧
- **离线导入与抽帧**：按固定源帧间隔抽取 PNG，支持取消；时间轴仅在选中视频时出现在画布底部
- **逐帧标注与插值**：复用矩形、多边形和关键点画布，按轨迹关键帧预览并应用线性插值，支持撤销/重做
- **统一保存与导出**：项目 JSON 保存全部图片、视频时间戳、源帧号和轨迹；COCO/VOC/YOLO 可导出为逐帧图片数据集，详见 [视频标注指南](docs/video-annotation.md)

**图片浏览**

- **打开图片文件夹**：支持 JPG / PNG / BMP，自动跳过损坏或空文件
- **删除图片**：在图片列表条目上右键选择「删除图片」，或在画布右键菜单中删除当前图片，也可用快捷键（默认 `F8`）；确认弹窗显示文件名、路径与标注数量，需等待 3 秒后用鼠标点击确认，源文件移入系统回收站（可从回收站恢复），该图片的标注与撤销记录同时清除，删除当前图片后自动切换到相邻图片；保存、导出或预打标进行中时不可删除；该功能依赖系统回收站，仅 Windows 支持
...
- **标签样例图**：可从当前项目的同标签标注中选择并裁剪为样例图，也可选择外部图片；支持预览、更换和清除，随标签保存并同步重命名；图片按名称保存在项目 `icon/` 目录，可从资源管理器批量维护，详见 [标签样例图](docs/label-samples.md)
...
- **从外部数据集创建项目**：手上只有 YOLO 格式的图片和标注目录时，无需本工具项目文件即可直接导入并生成项目；自动读取 `classes.txt` 生成标签（目录中暂无标注文件、仅含类别文件也能创建项目），导入后保存项目配置，再次打开该目录自动按本项目加载；打开尚未包含项目配置的图片目录时，若检测到 YOLO 标注会自动询问是否读取标签并当场创建项目
...
- **快捷键配置**：切图、缩放、工具切换、删除图片等通用快捷键可在配置面板中自定义并持久化保存；删除图片默认 `F8`，改绑时会校验与标签快捷键、固定按键的冲突
