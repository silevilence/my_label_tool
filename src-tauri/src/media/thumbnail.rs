...
fn write_atomically(cache_path: &Path, thumbnail: &DynamicImage) -> Result<(), String> {
    ...
    let part = cache_path.with_extension(format!("part-{}", std::process::id()));
    fs::write(&part, bytes).map_err(text::thumbnail_write_failed)?;
    fs::rename(&part, cache_path).map_err(text::thumbnail_write_failed)
}
