...
use crate::{
    ...
    media::prelabel::{
        ...
        pipeline::Detection,
        task::{CancellationToken, CancellationResult, TaskRegistry},
    },
...
/// checking the cancellation token between images. If cancelled, returns the images completed so
...
pub async fn run_prelabel_inference(
    ...
    outcome
        .map_err(text::prelabel_worker_failed)?
}
...
fn session_cache_key(model: &PrelabelModelConfig) -> Result<SessionCacheKey, String> {
    ...
    let modified_at = metadata.modified().map_err(|error| {
        text::prelabel_model_modified_time_failed(&model.path, error)
    })?;
    Ok(SessionCacheKey {
        model: model.clone(),
...
mod tests {
    ...
    use crate::media::prelabel::task::CancellationToken;
    ...
    fn progress_events_round_trip_a_completed_image() {
        ...
        assert_eq!(serde_json::to_value(&events[0]).unwrap()["event"], "modelLoading");
