...
export function ScriptPanel({ labels, onClose }: { labels: LabelConfig[]; onClose: () => void }) {
  ...
  return <Overlay onClose={onClose} canDismiss={!run.busy} label={text.title} size="wide" operationFeedback>
    <section className="flex max-h-[90vh] flex-col gap-4 overflow-y-auto rounded-xl border border-slate-700 bg-slate-900 p-5 text-slate-100">
      <header className="flex items-center justify-between"><div><h2 className="text-lg font-semibold">{text.title}</h2><p className="mt-1 text-xs text-slate-400">{text.scope(count)}</p></div><button disabled={run.busy} onClick={onClose} className="rounded border border-slate-600 px-3 py-2 disabled:opacity-40">{text.close}</button></header>
