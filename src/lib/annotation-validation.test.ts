...
describe("shared annotation constraints", () => {
  it("resolves unique names and ids without silently choosing duplicates", () => {
    ...
    expect(() => resolveLabel([...labels, { ...labels[0], id: "b" }], { name: "车辆" })).toThrow("LABEL_AMBIGUOUS");
  });
  it("validates types, membership, point counts and finite coordinates", () => {
    ...
    for (const bad of [null, {}, { ...shape, labelId: "" }, { ...shape, labelId: "missing" }, { ...shape, type: "other" }, { ...shape, points: [0] }, { ...shape, points: [0, 0, Infinity, 1] }]) expect(() => validateAnnotationCore(bad, labels)).toThrow();
    ...
    expect(() => validateAnnotationCore({ ...shape, type: "polygon", points: [0, 0, 1, 1, 2, 2] })).not.toThrow();
  });
});
