...
import { applyScriptPreview, createScriptSnapshot, formatScriptReport, prepareScriptResults } from "./script-execution";
import { parseNativeJsonImport } from "./importers";

const labels: LabelConfig[] = [{ id: "car", name: "车辆", shapeType: "rect", color: "#fff" }];
const shape: AnnotationShape = { id: "box", type: "rect", labelId: "car", points: [1, 2, 3, 4], frameIndex: 0 };
beforeEach(() => {
  useAnnotationStore.setState({ images: [{ path: "a", name: "a.png" }, { path: "b", name: "b.png" }, { path: "c", name: "c.png" }], selectedPath: "a", scopeStack: [{ kind: "project" }], frameIndices: {}, annotationsByImage: { a: [structuredClone(shape)], b: [structuredClone(shape)] }, selectedShapeId: "box", undoStack: [], redoStack: [], canUndo: false, canRedo: false });
});
...
describe("script transaction", () => {
  ...
  it("validates all results before writing and reports adds/removes/changes/skips", () => {
    const snapshot = createScriptSnapshot(labels);
    const preview = prepareScriptResults(snapshot, [{ imagePath: "a", annotations: [{ ...shape, points: [4, 3, 2, 1] }, { type: "rect", labelId: "car", points: [0, 0, 1, 1] }] }, { imagePath: "b", annotations: [] }], () => "new");
    expect(preview.report).toEqual({ images: 3, submitted: 2, added: 1, removed: 1, changed: 1, skipped: 1 });
    ...
    expect(parseNativeJsonImport(saved).images[0].annotations).toEqual(useAnnotationStore.getState().annotationsByImage.a);
  });
  it("keeps existing batch undo per-image and prunes deleted images from script history", () => {
    const store = useAnnotationStore.getState();
    store.insertAnnotationsBatch([{ imagePath: "a", annotations: [] }, { imagePath: "b", annotations: [] }], "replace");
    store.undo();
    expect(useAnnotationStore.getState().annotationsByImage.a).toEqual([]);
    expect(useAnnotationStore.getState().annotationsByImage.b).toEqual([shape]);
    const preview = prepareScriptResults(createScriptSnapshot(labels), [{ imagePath: "a", annotations: [shape] }, { imagePath: "b", annotations: [] }]);
    applyScriptPreview(preview, labels, "script");
    store.removeImage("a"); store.undo();
  ...
  it("rejects stale annotations, scope and labels before applying", () => {
    const preview = prepareScriptResults(createScriptSnapshot(labels), [{ imagePath: "a", annotations: [] }]);
  ...
  it("does not treat object property ordering as a change", () => {
    const reordered = { points: [1, 2, 3, 4], labelId: "car", type: "rect", frameIndex: 0, id: "box" };
    const preview = prepareScriptResults(createScriptSnapshot(labels), [{ imagePath: "a", annotations: [reordered] }]);
  ...
  it("aggregates failures and agrees with native core constraints", () => {
    const snapshot = createScriptSnapshot(labels);
    for (const invalid of [{ ...shape, labelId: "missing" }, { ...shape, points: [] }, { ...shape, points: [NaN, 2, 3, 4] }, { ...shape, type: "point" }]) {
      expect(() => prepareScriptResults(snapshot, [{ imagePath: "a", annotations: [invalid] }])).toThrow();
      expect(() => parseNativeJsonImport(JSON.stringify({ labels, images: [{ name: "a.png", annotations: [invalid] }] }))).toThrow();
    }
    for (const invalid of [{ ...shape, id: "" }, { ...shape, attributes: { x: null } }, { ...shape, frameIndex: -1 }]) expect(() => prepareScriptResults(snapshot, [{ imagePath: "a", annotations: [invalid] }])).toThrow();
    expect(() => prepareScriptResults(snapshot, [{ imagePath: "outside", annotations: [] }])).toThrow("outside");
    expect(() => prepareScriptResults(snapshot, [{ imagePath: "a", annotations: [shape, shape] }])).toThrow();
    expect(() => prepareScriptResults(snapshot, [{ imagePath: "a", annotations: [] }, { imagePath: "a", annotations: [] }])).toThrow();
    expect(useAnnotationStore.getState().annotationsByImage.a).toEqual([shape]);
  });
});
