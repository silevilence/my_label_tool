...
export function validateScriptLimits(value: ScriptLimits): void {
  if (
    !Number.isSafeInteger(value.maxMemoryMiB) ||
    value.maxMemoryMiB < spec.memory.min ||
    value.maxMemoryMiB > spec.memory.max ||
    !Number.isSafeInteger(value.timeoutSeconds) ||
    value.timeoutSeconds < spec.timeout.min ||
    value.timeoutSeconds > spec.timeout.max
  )
    throw new Error(
      text.invalidLimits(spec.memory.min, spec.memory.max, spec.timeout.min, spec.timeout.max),
    );
}
