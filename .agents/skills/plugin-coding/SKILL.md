...
- **权限最小化**：插件访问文件/网络/项目数据必须经宿主协议代理与权限模型（`fs.read`/`fs.write` 目录级授予、`network` 默认拒绝、`spawn` 不开放）；Windows 代码型插件必须在 AppContainer 中运行，只授予插件包、声明使用的系统解释器运行目录只读/执行与协议 stdio，宿主环境按白名单重建，初始化失败时拒绝启动，进程结束或启动失败时撤销临时 ACL/profile；环境变量离线标志不算安全边界。禁止绕过权限模型直接向插件暴露 Tauri commands、文件句柄或底层系统能力。
...
- **测试要求**：manifest 校验、协议解析、权限判定、配置迁移、故障隔离逻辑必须有单元测试；权限隔离验收必须包含插件直接读取包外文件、读取宿主秘密环境变量和直接 socket 连接的 OS 级拒绝测试，并真实启动包内 EXE 与系统 Python 示例，不能用代理拒绝或环境变量替代；新增错误码、消息类型、协议行为必须同步 conformance 测试与文档。行为验证（单测/冒烟）的测试脚本与运行结果必须保留在可复核位置，禁止跑完即删、只留口头结论——审查方要能复跑或查看证据。
