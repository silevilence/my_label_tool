...
├── src/                        # React 前端
│   ├── components/
│   │   ├── canvas/             # Konva 画布、标注图形组件
│   │   ├── toolbar/            # 工具栏、标签选择面板
│   │   ├── sidebar/            # 图片列表、标注列表
│   │   └── settings/           # 快捷键配置、标签配置、导出模板配置 UI
│   ├── store/                  # Zustand stores（按领域拆分：annotation/labels/settings/project）
│   ├── hooks/                  # 自定义 hooks（如 useShortcut、useImageZoom）
│   ├── types/                  # TypeScript 类型定义（标注数据结构、配置结构）
│   ├── lib/
│   │   ├── exporters/          # 各导出格式实现（coco.ts / voc.ts / yolo.ts / custom.ts）
│   │   └── tauri-api.ts        # 封装对 Rust command 的调用
│   └── App.tsx
├── src-tauri/                  # Rust 后端
│   ├── src/
│   │   ├── commands/           # 各 Tauri command（文件IO、导出、抽帧）
│   │   ├── models/             # 与前端 types 对应的 Rust struct（serde）
│   │   └── main.rs
│   ├── Cargo.toml
│   └── tauri.conf.json
├── docs/                        # 设计文档、导出格式说明
...
**原则**：标注数据结构（`AnnotationShape`、`LabelConfig`、`ExportTemplate` 等）必须在 `src/types/` 中先定义清楚，Rust 端 `models/` 保持字段一一对应，避免前后端数据结构漂移。
...
1. **M1 最小可用**：打开图片文件夹 → 画矩形框 → 绑定固定标签 → 导出 JSON
2. **M2 配置系统**：标签配置面板（增删改标签/颜色/快捷键）、导出模板选择（预置 COCO/VOC/YOLO）
3. **M3 交互打磨**：鼠标滚轮缩放（以鼠标位置为中心）、Ctrl+滚轮微调、方向键切图、数字键速切标签、撤销/重做栈
4. **M4 打包验证**：`tauri build` 产出 Windows exe，在无开发环境的机器上验证双击即用、启动速度
5. **M5 图形扩展**：多边形、关键点标注支持
6. **M6 视频标注**（后期）：视频抽帧、时间轴 UI、跨帧标注插值
...
- 禁止在未确认 `AnnotationShape` 坐标系约定（归一化 vs 像素）前，新增涉及坐标计算的功能。
