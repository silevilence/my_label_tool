...
图片/视频标注桌面工具（Tauri 2 + React）。本上下文覆盖五个领域的语言约定：「预打标链路」中 `.pt` 模型自动转换、插件系统、前端的交互层（遮罩、快捷键、操作、选择与作用域）、受控环境下的宿主文件读取，以及服务端形态（局域网多人访问）。
...
**遮罩 (Overlay)**:
盖在画布之上、需要占用输入的一层界面（弹窗、菜单、浮层）。分两类：阻塞级（独占全部快捷键与焦点）与轻量级（只吞掉画布写操作类动作，保留保存、缩放等全局动作）。
_Avoid_: 弹窗、浮层、Modal

**输入门禁 (Input Gate)**:
判断一次键盘或指针事件当前是否应由某个消费者处理的依据；由遮罩栈与焦点位置共同决定，不由消费者各自维护名单。
_Avoid_: 快捷键开关、启用条件

**快捷键动作 (Shortcut Action)**:
可被键盘触发的一项行为，带名称、默认键位、作用范围（画布 / 全局）与是否可改绑。
_Avoid_: 快捷键、绑定、键位

**作用域 (Scope)**:
决定「上一张 / 下一张 / 下一个未标注」在哪个有序集合内移动的上下文：项目序、搜索结果、单个视频的帧序。
_Avoid_: 过滤、筛选、分组

**作用域栈 (Scope Stack)**:
作用域的有序集合，栈顶生效；打开搜索或进入某个视频的帧序时压入，退出时弹出。栈顶作用域必须对用户可见、可退出。
_Avoid_: 导航模式、浏览状态

**操作 (Operation)**:
一项可观察、可取消的长任务（导出、预打标、视频抽帧、模型与运行时下载、保存、删除）。带名称、进度、取消句柄与占用的资源；消息按操作归属。
_Avoid_: 任务、作业、进程

**操作资源 (Operation Resource)**:
操作占用的互斥对象（如项目标注、导出目录、视频帧、模型下载、ONNX 运行时）。占用同一资源的操作不得并发。
_Avoid_: 锁、互斥量、占用标记

## 宿主文件读取 (Host File Reading)

**受控文本文件 (Protected Text File)**:
被终端透明加密策略覆盖、非白名单进程只能读到密文的文本文件（txt/json/xml 等）。
_Avoid_: 加密文件、密文文件

**透明加密 (Transparent Encryption)**:
终端安全客户端在文件读写路径上按策略自动加解密的环境行为：白名单进程读得明文，其他进程读得密文，任何进程的写入一律以密文落盘。
_Avoid_: 文件加密、加壳、加密驱动

**读取适配器 (Read Adapter)**:
宿主读取文本文件内容的可替换执行方式，内置原生模式、Python 模式与命令模式三种预设。
_Avoid_: 解密器、读取 Hook、读取代理

**原生模式 (Native Mode)**:
由宿主进程直接读取文件的读取适配器预设，也是非受控环境下的默认模式。
_Avoid_: 直读模式、普通模式

**Python 模式 (Python Mode)**:
以配置的 Python 解释器执行宿主内置脚本完成读取、明文经标准输出管道返回的读取适配器预设。
_Avoid_: 脚本模式、python 代理

**命令模式 (Command Mode)**:
执行用户配置的可执行文件与参数数组模板完成读取的读取适配器预设，参数不经 shell 解析。
_Avoid_: bash 模式、外部命令模式

**读取自检 (Read Self-Check)**:
用当前适配器读回宿主自己刚写出的临时文本文件并比对内容的健康检查，用于验证配置与发现策略收紧。
_Avoid_: 探针、canary 检测、预检

## 服务端形态 (Server Mode)

**服务端形态 (Server Mode)**:
应用以局域网服务器运行、浏览器作为客户端的部署形态，与桌面形态并存并共用同一前端与核心逻辑。
_Avoid_: Web 版、云端版、SaaS

**桌面形态 (Desktop Form)**:
现有的 Tauri 窗口形态：单机离线运行，直接访问本机文件系统。与服务端形态的唯一分岔处是桥接层。
_Avoid_: 本地版、客户端

**服务端账号 (Server Account)**:
服务端本地维护的用户名 + 口令身份，用于操作归属、取消权与并发冲突提示；桌面形态不需要账号。
_Avoid_: 用户、账号体系、登录态

**工作区根目录 (Workspace Root)**:
服务端管理员配置的白名单目录，项目目录必须位于其中；决定磁盘上哪些位置对浏览器客户端可见。
_Avoid_: 数据目录、根路径、共享目录

**操作归属 (Operation Ownership)**:
操作与其发起者（服务端形态下为账号，桌面形态下为本机会话）的绑定关系，决定谁可取消、断线后是否继续。
_Avoid_: 任务所有权、锁、占用者
