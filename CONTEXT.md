# my_label_tool 标注工具上下文

图片/视频标注桌面工具（Tauri 2 + React）。本上下文覆盖七个领域的语言约定：「预打标链路」中 `.pt` 模型自动转换、插件系统、脚本（Lua 数据变换）、前端的交互层（遮罩、快捷键、操作、选择与作用域）、标签样例图、受控环境下的宿主文件读取，以及服务端形态（局域网多人访问）。

## Language

**转换方法 (Conversion Method)**:
`.pt` 模型转换为 ONNX 时采用的执行环境方式。取值：`yolo-cli`（PATH 上的 yolo 命令直接执行）、`python-ultralytics`（PATH 上的 Python 且已安装 yolo 相关模块）、`uvx-yolo`（PATH 上的 uv，经 `uvx` 运行 yolo 命令）。
_Avoid_: 转换器、converter、执行器

**转换参数 (Conversion Parameters)**:
发起 .pt 转换时可由用户确认或修改的参数。当前为 `imgsz`（默认 640，32 的倍数）与 `simplify`（默认关闭）；弹窗始终预填默认值，不持久化。
_Avoid_: 配置项、导出参数

## 插件系统 (Plugin System)

**插件 (Plugin)**:
独立打包、可安装启用的功能扩展单元；zip 格式打包，后缀不限。按扩展类型分数据型（预置标签）与代码型（导出格式、外部预打标程序）。
_Avoid_: 扩展、模块、addon

**扩展类型 (Extension Kind)**:
插件在清单中声明的能力类别。v1 取值：`label-preset`（预置标签，纯数据）、`exporter`（保存/导出格式扩展）、`prelabel`（外部预打标程序）。
_Avoid_: 插件分类、category

**插件清单 (Manifest)**:
插件包内的声明文件，包含 ID、名称、插件版本、宿主 API 兼容区间、运行时与入口命令、能力声明、权限清单、配置版本，以及可选的调用超时 `timeoutMs`（缺省 30000 毫秒，上限 300000 毫秒）。
_Avoid_: 元数据、插件配置

**插件 ID (Plugin ID)**:
反向域名命名空间的永久稳定标识（如 `dev.acme.xxx`）。名称可改，ID 一经发布不得复用。
_Avoid_: 插件名、slug

**标签预置 (Label Preset)**:
`label-preset` 数据型插件在包根 `labels.json` 提供的只读 `LabelTemplate`。模板与标签 ID
使用插件 ID 命名空间；加载后进入项目标签快照，插件更新、停用或卸载不回写已加载项目。
_Avoid_: 标签包、在线模板

**插件导出格式 (Plugin Export Format)**:
`exporter` 代码型插件在 manifest `exporterOptions.formats` 中静态声明的格式。插件通过
`exporter.export` 返回相对路径与文件内容，宿主先校验全部文件再写入用户选择目录；
插件不接触输出目录。

`prelabel` 代码型插件是与内置 ONNX 管线并存的预打标来源。插件只接收项目相对图片
路径，通过 `fs.read:%PROJECT%` 代理读取图片，并返回原图像素坐标的标注图形；宿主
负责类别映射、结果校验、按图片合并与撤销历史。
_Avoid_: 外部导出器、动态格式

**宿主 API 版本 (Host API Version)**:
宿主暴露给插件的能力接口版本号，独立递增，与主程序版本号互不绑定（主程序发版不得顺手同步 bump）。分两层：整体版本（覆盖 `hello`/`fs`/`config.migrate` 等基础能力）与业务能力版本（`exporter`/`prelabel` 各自独立）。同一版本号内宿主承诺增量兼容；破坏性变更必须开新版本号，旧版本实现按弃用期保留后移除，宿主按插件声明的目标版本分派对应实现（见 ADR 0007）。
_Avoid_: 接口版本、API 兼容区间

**目标版本 (Target API Version)**:
插件清单 `apiVersion.min` 声明的宿主 API 版本，表示插件编写所针对的确切版本；宿主按此版本分派对应实现，版本不匹配时以 `API_VERSION_UNSUPPORTED` 拒绝。
_Avoid_: 兼容范围、最低版本

**能力 (Capability)**:
插件启动握手时声明的实际支持项：标注类型、批处理、进度、取消、配置迁移。宿主以能力声明为准，不以版本号推断。
_Avoid_: 特性、功能列表

**权限授予 (Permission Grant)**:
宿主授予插件的资源访问权。v1 权限面为 `fs.read` / `fs.write`（安装授权时把目录占位符解析为绝对路径后落盘）与 `network`，默认全部拒绝；v1 不提供网络代理。Windows 代码型插件由 AppContainer 强制限制为插件包、声明使用的系统解释器运行目录只读/执行与协议 stdio，其他包外文件必须经宿主代理，宿主环境按白名单重建，v1 不授予网络 capability。
_Avoid_: 权限、access

**授权 (Authorization)**:
安装或权限变更时，用户对权限清单的逐项确认动作。未经授权的访问在运行时返回 `PERMISSION_DENIED`。
_Avoid_: 审批、放行

**插件配置 (Plugin Config)**:
项目保存的插件状态三元组 `pluginId + configVersion + config`；config 对宿主不透明，宿主不解释其内容。
_Avoid_: 插件设置、配置项

**配置迁移 (Config Migration)**:
插件配置版本落后时，宿主经 `config.migrate` 调用由插件升级配置的过程。失败时配置标记「待迁移」、相关能力降级，项目照常打开。
_Avoid_: 升级、数据迁移

**安全模式 (Safe Mode)**:
手动开关的排查状态：禁用全部代码型插件，保留数据型插件（预置标签）。
_Avoid_: 恢复模式、诊断模式

**自动禁用 (Auto-Disable)**:
同一插件连续 3 次调用失败或启动崩溃后进入禁用态；需用户手动重新启用。
_Avoid_: 熔断、封禁

**标准错误码 (Error Code)**:
协议规定的错误标识集合：`PARSE_ERROR`、`PROTOCOL_ERROR`、`METHOD_NOT_FOUND`、`PERMISSION_DENIED`、`TIMEOUT`、`CANCELLED`、`INTERNAL_ERROR`、`CONFIG_MIGRATION_REQUIRED`、`INVALID_ARGUMENT`、`API_VERSION_UNSUPPORTED`（协商阶段插件声明版本与宿主支持版本不匹配）。
_Avoid_: 异常、错误信息

**调用 (Call)**:
宿主向插件进程发起的一次请求-响应事务；默认超时 30 秒，可关联进度事件与取消控制。
_Avoid_: 请求、任务

**注册 (Registration)**:
插件安装流程的收尾环节：发现 → 静态校验 → 兼容性协商 → 授权 → 注册 → 启用。
_Avoid_: 安装流程、登记

## 脚本 (Scripting)

**脚本 (Script)**:
应用内编写、显式运行、由独立宿主进程执行的 Lua 数据变换程序；处理对象是当前项目作用域内的图片与标注，不接触图片文件本身。
_Avoid_: 插件、宏、批处理任务

**脚本宿主 (Script Host)**:
承载 Lua 解释器的独立进程；与主程序只交换「请求 / 响应 / 进度 / 取消」，不具备文件与网络能力。
_Avoid_: 脚本引擎、运行时、沙箱

**脚本命令 (Script Command)**:
宿主进程注入脚本运行环境的本地函数（取图片集、读标注、提交结果、上报进度、写日志）；提交只登记结果，不直接写回。与 Tauri command 无关。
_Avoid_: 宿主 API、插件方法、接口

**脚本库 (Script Library)**:
应用数据目录下与机器绑定的脚本集合（一脚本一份 `.lua`）。脚本随人走，项目配置永不携带脚本内容。
_Avoid_: 模板、插件包

**脚本运行 (Script Run)**:
一次脚本执行的完整事务：按作用域栈顶取快照 → 分块执行 → 结果缓冲 → 全部成功后原子写回。取消即终止宿主进程，标注零变化。
_Avoid_: 脚本任务、批处理、作业

## 标签样例图 (Label Samples)

**标签样例图 (Label Sample)**:
标签在项目中的代表图，保存在项目 `icon/` 目录、文件名与标签名一一对应（PNG / JPG / JPEG / BMP，匹配不区分大小写）。它是目录约定而不是数据字段：不进入 `LabelConfig`、`LabelTemplate` 或项目配置，也不参与标注导出。
_Avoid_: 标签图标、缩略图、封面图

**样例图裁剪 (Sample Crop)**:
用项目内该标签的既有标注区域生成样例图的方式：矩形按框截取、多边形取外接矩形、关键点取周围 64×64 像素；只有被选中的候选才会写出临时 PNG。
_Avoid_: 截图、截屏、裁剪工具

## 前端交互 (Frontend Interaction)

**遮罩 (Overlay)**:
盖在画布之上、需要占用输入的一层界面（弹窗、菜单、浮层）。分两类：阻塞级（独占全部快捷键与焦点）与轻量级（只吞掉画布写操作类动作，保留保存、缩放等全局动作）。
_Avoid_: 弹窗、浮层、Modal

**输入门禁 (Input Gate)**:
判断一次键盘或指针事件当前是否应由某个消费者处理的依据；由遮罩栈与焦点位置共同决定，不由消费者各自维护名单。
_Avoid_: 快捷键开关、启用条件

**快捷键动作 (Shortcut Action)**:
可被键盘触发的一项行为，带名称、默认键位、作用范围（画布 / 全局）与是否可改绑。
_Avoid_: 快捷键、绑定、键位

**作用域 (Scope)**:
决定「上一张 / 下一张 / 下一个未标注」在哪个有序集合内移动的上下文：项目序、搜索结果、单个视频的帧序。
_Avoid_: 过滤、筛选、分组

**作用域栈 (Scope Stack)**:
作用域的有序集合，栈顶生效；打开搜索或进入某个视频的帧序时压入，退出时弹出。栈顶作用域必须对用户可见、可退出。
_Avoid_: 导航模式、浏览状态

**操作 (Operation)**:
一项可观察、可取消的长任务（导出、预打标、视频抽帧、模型与运行时下载、保存、删除）。带名称、进度、取消句柄与占用的资源；消息按操作归属。
_Avoid_: 任务、作业、进程

**操作资源 (Operation Resource)**:
操作占用的互斥对象（如项目标注、导出目录、视频帧、模型下载、ONNX 运行时）。占用同一资源的操作不得并发。
_Avoid_: 锁、互斥量、占用标记

## 宿主文件读取 (Host File Reading)

**受控文本文件 (Protected Text File)**:
被终端透明加密策略覆盖、非白名单进程只能读到密文的文本文件（txt/json/xml 等）。
_Avoid_: 加密文件、密文文件

**透明加密 (Transparent Encryption)**:
终端安全客户端在文件读写路径上按策略自动加解密的环境行为：白名单进程读得明文，其他进程读得密文，任何进程的写入一律以密文落盘。
_Avoid_: 文件加密、加壳、加密驱动

**读取适配器 (Read Adapter)**:
宿主读取文本文件内容的可替换执行方式，内置原生模式、Python 模式与命令模式三种预设。
_Avoid_: 解密器、读取 Hook、读取代理

**原生模式 (Native Mode)**:
由宿主进程直接读取文件的读取适配器预设，也是非受控环境下的默认模式。
_Avoid_: 直读模式、普通模式

**Python 模式 (Python Mode)**:
以配置的 Python 解释器执行宿主内置脚本完成读取、明文经标准输出管道返回的读取适配器预设。
_Avoid_: 脚本模式、python 代理

**命令模式 (Command Mode)**:
执行用户配置的可执行文件与参数数组模板完成读取的读取适配器预设，参数不经 shell 解析。
_Avoid_: bash 模式、外部命令模式

**读取自检 (Read Self-Check)**:
用当前适配器读回宿主自己刚写出的临时文本文件并比对内容的健康检查，用于验证配置与发现策略收紧。
_Avoid_: 探针、canary 检测、预检

## 服务端形态 (Server Mode)

**服务端形态 (Server Mode)**:
应用以局域网服务器运行、浏览器作为客户端的部署形态，与桌面形态并存并共用同一前端与核心逻辑。
_Avoid_: Web 版、云端版、SaaS

**桌面形态 (Desktop Form)**:
现有的 Tauri 窗口形态：单机离线运行，直接访问本机文件系统。与服务端形态的唯一分岔处是桥接层。
_Avoid_: 本地版、客户端

**服务端账号 (Server Account)**:
服务端本地维护的用户名 + 口令身份，用于操作归属、取消权与并发冲突提示；桌面形态不需要账号。
_Avoid_: 用户、账号体系、登录态

**工作区根目录 (Workspace Root)**:
服务端管理员配置的白名单目录，项目目录必须位于其中；决定磁盘上哪些位置对浏览器客户端可见。
_Avoid_: 数据目录、根路径、共享目录

**操作归属 (Operation Ownership)**:
操作与其发起者（服务端形态下为账号，桌面形态下为本机会话）的绑定关系，决定谁可取消、断线后是否继续。
_Avoid_: 任务所有权、锁、占用者
